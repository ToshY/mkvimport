...
from mkvimport.args import (
    files_in_dir,
    InputPathChecker,
    OutputPathChecker,
    OptionalValueChecker,
)
from mkvimport.helper import combine_arguments_by_batch, mimetype_by_extension
from mkvimport.process import ProcessCommand
...
def strip_file_attachments(input_file: Path) -> Path:
    ...
    Raises:
        Exception: If there is an error while stripping the attachments, subtitles, tags, and chapters, an exception
        is raised.
    """
...
def remove_suffix_from_string(input_string: str, suffix: str) -> str:
    """
    Remove the specified suffix from the input string if it exists.

    Parameters:
        input_string (str): The input string from which the suffix is removed.
        suffix (str): The suffix to remove from the input string.

    Returns:
        str: The input string with the specified suffix removed.
    """
...
@click.command(
    ...
    epilog="Repository: https://github.com/ToshY/mkvimport",
)
@click.option(
    "--input-path",
    "-i",
...
def cli(input_path, output_path, mode):
